...
const config = require('./config.json'); // Require config file with token and prefix
...
}); // This is for command loading in the handler file, one firing for each cmd
...
client.on('message', async message => {
    ...
    if (cmd.length === 0) return; // If no command, then return
